...
export async function GET() {
  try {
    ...
    const markedRegularOrders = regularOrders.map(order => ({
      ...
      total: order.total,
      createdAt: order.created_at,
      trackingId: order.tracking_number || order.tracking_id,
      trackingStatus: order.tracking_status,
      estimatedDelivery: order.estimated_delivery,
      trackingUpdates: typeof order.tracking_updates === 'string' ? JSON.parse(order.tracking_updates) : order.tracking_updates || [],
