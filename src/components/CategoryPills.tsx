...
export default function CategoryPills() {
  ...
  useEffect(() => {
    // Fetch top subcategories when component mounts
    fetch('/api/top-subcategories')
      .then(res => res.json())
      .then(data => setTopSubcategories(data))
      .catch(err => console.error('Failed to fetch top subcategories:', err))
  }, [])

  const categoriesToShow = useMemo(() => {
    if (!activeCategory || !mainCategories.includes(activeCategory)) {
      // Show top 20 subcategories by product count
      return ['All', ...topSubcategories.map(sub => sub.name)];
    }
  ...
  }, [activeCategory, getSubcategories, topSubcategories]);
